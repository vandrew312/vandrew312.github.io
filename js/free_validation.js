...
var validate = {
	check : function (cform, imin, imax) {
		var error_message = 'Пожалуйста исправьте следующие ошибки:\n\n';
		message_error='';
		var mess_part = '';
		var to_focus = '';
		var tmp = true;
		...
		for (var i = imin; i <= imax; i++) {
			if (this.checkit(required.field[i][0], required.field[i][1], cform)) {}
				...
				error_message = error_message  + required.field[i][2] + ' должно быть заполнено правильно!\n' + message_error;
				message_error= '';
	...
	checkit : function (cvalue, ctype, cform) {
		if (ctype == "NOT_EMPTY") {
			if ((this.trim($$(cvalue)).length < 2) || (this.trim($$(cvalue)).length > 20) )  {
				message_error= 'Должно быть от 2 до 20 символов! \n';
				return false;
			} else {
				return true;
			}
			...
			if ((this.trim($$(cvalue)).length < 7) || (this.trim($$(cvalue)).length > 13) ) {
				message_error= 'Должно быть от 7 до 13 цифр! \n';
				return false;
			} else {
				return true;
			}
			...
			if ((this.trim($$(cvalue)).length < 2) || (this.trim($$(cvalue)).length > 30) ) {
				message_error= 'Должно быть от 2 до 30 символов! \n';
				return false;
			} else {
				return true;
			}
			...
			if ((this.trim($$(cvalue)).length < 2) || (this.trim($$(cvalue)).length > 2000) ) {
				message_error= 'Должно быть от 2 до 2000 символов! \n';
				return false;
			} else {
				return true;
			}
